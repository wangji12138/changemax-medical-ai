package com.example.wangji.changemax.activity.app_introduction.widget;
...
import com.example.wangji.changemax.activity.app_introduction.widget.holder.ShapeHolder;
...
public class CircleIndicator extends View {
    ...
    //配置列表
    private int mCurItemPosition;//CurItem位置
    private float mCurItemPositionOffset;//CurItem位置偏移
    private float mIndicatorRadius;//指标半径
    private float mIndicatorMargin;//指标余量
    private int mIndicatorBackground;//指标背景
    private int mIndicatorSelectedBackground;//m指标选定的背景
    private Gravity mIndicatorLayoutGravity;//m指标布局重力
    private Mode mIndicatorMode;//m指标Mod

    //默认值
    private final int DEFAULT_INDICATOR_RADIUS = 5;//默认指示器半径
    private final int DEFAULT_INDICATOR_MARGIN = 40;//默认指标余量
    private final int DEFAULT_INDICATOR_BACKGROUND = Color.BLUE;//默认指示器背景
    private final int DEFAULT_INDICATOR_SELECTED_BACKGROUND = Color.RED;//默认指示器选择背景
    private final int DEFAULT_INDICATOR_LAYOUT_GRAVITY = Gravity.CENTER.ordinal();//默认指标布局重力
    private final int DEFAULT_INDICATOR_MODE = Mode.SOLO.ordinal();//默认指示模式
    ...
    private void createTabItems() {
        for (int i = 0; i < viewPager.getAdapter().getCount(); i++) {
            ...
            paint.setColor(mIndicatorBackground);//设置指标背景颜色
    ...
    private void createMovingItem() {
        ...
        Paint paint = drawable.getPaint();
        paint.setColor(mIndicatorSelectedBackground);//设置指标颜色
        paint.setAntiAlias(true);
